...
export function MotionCard({ children, onPress, style }: { children: ReactNode; onPress?: PressableProps["onPress"]; style?: ViewStyle }) {
  ...
  const translateY = motion.interpolate({ inputRange: [0, 1], outputRange: [0, -3] });
  const scale = motion.interpolate({ inputRange: [0, 1], outputRange: [1, 1.012] });
...
const styles = StyleSheet.create({ pressable: { borderRadius: 18 }, pressed: { opacity: 0.86 }, });
