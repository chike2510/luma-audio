...
const titles: Record<string, { label: string; title: string; accent: string }> = {
  beat: { label: "DRUMS", title: "Beat", accent: LUMA.cyan },
  midi: { label: "KEYS", title: "MIDI", accent: LUMA.violet },
  mix: { label: "MIX", title: "Mixer", accent: LUMA.green },
  ai: { label: "AI CO-PRODUCER", title: "Co-producer", accent: LUMA.violet },
  export: { label: "EXPORT", title: "Export", accent: LUMA.cyan },
};
...
function BeatScreen({ accent }: { accent: string }) {
  ...
  return <>
    <View style={styles.headingRow}><View><Eyebrow>BEAT MAKER</Eyebrow><Text style={styles.heading}>Pattern 01</Text><Text style={styles.subheading}>16 steps · 124 BPM · C minor</Text></View><Text style={[styles.bpm, { color: accent }]}>124<Text style={styles.bpmUnit}> BPM</Text></Text></View>
...
function MidiScreen({ accent }: { accent: string }) {
  ...
  return <>
    <View style={styles.headingRow}><View><Eyebrow>MIDI PIANO ROLL</Eyebrow><Text style={styles.heading}>Piano roll</Text><Text style={styles.subheading}>Grand Piano · C major · 1/16 grid</Text></View><Text style={[styles.bpm, { color: accent }]}>C Maj<Text style={styles.bpmUnit}> KEY</Text></Text></View>
...
function MixScreen({ accent }: { accent: string }) {
  return <>
    <View style={styles.headingRow}><View><Eyebrow>MIXER & EFFECTS</Eyebrow><Text style={styles.heading}>Mix balance</Text><Text style={styles.subheading}>Levels, pan, and bus processing</Text></View><Text style={[styles.bpm, { color: accent }]}>-1.2<Text style={styles.bpmUnit}> dB</Text></Text></View>
...
function AiScreen({ accent }: { accent: string }) {
  return <>
    <View style={styles.aiHero}><Eyebrow>AI CO-PRODUCER</Eyebrow><Text style={styles.heading}>Luma co-producer</Text><Text style={styles.subheading}>Suggestions are editable and reversible.</Text><View style={styles.aiOrb}><Text style={[styles.aiOrbText, { color: accent }]}>✦</Text></View></View>
    <Text style={styles.question}>Choose an action</Text>
...
function ExportScreen({ accent, state, onExport }: { accent: string; state: "idle" | "rendering" | "success"; onExport: () => void }) {
  return <>
    <View style={styles.headingRow}><View><Eyebrow>EXPORT SETTINGS</Eyebrow><Text style={styles.heading}>Render the session</Text><Text style={styles.subheading}>Choose a format for the final bounce.</Text></View><Text style={[styles.bpm, { color: accent }]}>WAV</Text></View>
